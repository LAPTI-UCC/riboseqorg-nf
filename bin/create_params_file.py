...
def main(args):
    ...
    parameter_dict['outdir'] = '/'.join(args.sample_sheet.split('/')[:-1])
    parameter_dict['sample_sheet'] = args.sample_sheet

    with open(f"{args.outdir}/params.config", 'w') as f:
        f.write("params {\n")
        f.write(f'\toutdir = "{parameter_dict["outdir"]}"\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--outdir', help='Output dir name', required=True)
